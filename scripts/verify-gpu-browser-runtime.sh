...
runtime_dir="$(mktemp -d "${TMPDIR:-/tmp}/stockgod-gpu-browser.XXXXXX")"
...
if (( node_major < 18 )); then
	echo "GPU browser acceptance requires Node.js 18 or newer." >&2
	exit 1
fi

echo "Building frontend and one temporary backend for isolated desktop and mobile GPU acceptance."
(
...
for device in desktop mobile; do
	...
	(
		...
		env PORT="$port" \
			...
			STOCKGOD_ADMIN_TOKEN="gpu-browser-$device-$RANDOM-$$" \
			STOCKGOD_GPU_RUNTIME_FIXTURE="temporary-local-acceptance-v1" \
			...
			OPENAI_COMPATIBLE_API_KEY="gpu-runtime-no-network" \
			"$binary_path"
	) >"$server_log" 2>&1 &
	server_pid=$!
	...
	for _ in $(seq 1 100); do
		...
		if ! kill -0 "$server_pid" 2>/dev/null; then
			echo "Temporary $device GPU backend exited before health check" >&2
	...
	if [[ "$ready" != "true" ]]; then
		echo "Temporary $device GPU backend did not become healthy" >&2
		tail -80 "$server_log" >&2
		exit 1
	fi
	...
	(
		cd "$repo_root/app/frontend"
		GPU_RUNTIME_WRITES=true GPU_BASE_URL="http://127.0.0.1:$port" GPU_VIEWPORT="$device" \
			"$node_bin" scripts/gpu-runtime-audit.mjs
	)
	kill "$server_pid" 2>/dev/null || true
	wait "$server_pid" 2>/dev/null || true
	server_pid=""
...
echo "PASS: isolated desktop and mobile GPU runtime flows used real service, persistence, router and admin endpoints."
